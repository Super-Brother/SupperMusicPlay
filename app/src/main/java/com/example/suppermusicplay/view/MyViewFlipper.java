package com.example.suppermusicplay.view;
  ...
  public boolean onTouch(View view, MotionEvent event) {
    ...
    switch (event.getAction()) {
      ...
      case MotionEvent.ACTION_UP:
        ...
        if (pageOffset > 0.5) {
          //切换 动画自动偏移剩余的距离
          final ValueAnimator animator = ValueAnimator.ofFloat(dx, isNext ? -flipper_width : flipper_width);
          animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
              getCurrentView().setTranslationX((Float) animator.getAnimatedValue());
              getOtherView().setTranslationX((Float) animator.getAnimatedValue()
                      + (isNext ? flipper_width : -flipper_width));
              if (Math.abs((Float) animator.getAnimatedValue()) == flipper_width) {
                //动画执行完成
                if (isNext) {
          ...
          //回弹
          final ValueAnimator animator = ValueAnimator.ofFloat(dx, 0);
          animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
              getCurrentView().setTranslationX((Float) animator.getAnimatedValue());
